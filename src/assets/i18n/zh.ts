export const zh = {
  common: {
    followers: '订阅',
    releaseToRefresh: '释放刷新内容',
    beingRefresh: '刷新中...',
    cancel: '取消',
    ok: '确定',
    confirm: '确认',
    des: '清空 <strong>所有缓存数据</strong>!!!',
    des1: '确认频道',
    server: '出版号DID',
    description: '描述',
    feeds: 'Feeds',
    just: '刚刚',
    yesterday: '昨天',
    unsubscribe: '取消订阅',
    share: '分享',
    addressinvalid: '地址无效',
    comingSoon: '敬请期待...',
    noImageSelected: '未选择任何图像',
    loggingIn: '正在登录',
    des2: '确认要返回主页面吗？',
    connectionError: '当前无网络，请稍候再试',
    success: '成功',
    fail: '失败',
    unknownError: '未知错误',
    loadingText: '加载更多数据...',
    noElaAddress: '频道所有者尚未设置打赏地址',
    copysucceeded: '复制成功',
    des4: '您的捐款将寄至ELA地址：',
    amountError: '请输入正确的数额',
    waitMoment: '请稍候...',
    save: '保存',
    channel: '频道',
    image: '图片',
    nochanges: '未做任何更改',
    duplicate: '您不得创建包含重复内容的推文。',
    editpost: '编辑微说',
    removepost: '删除微说',
    editcomment: '编辑微评',
    removecomment: '删除微评',
    edit: '(已编辑)',
    deleted: '(微说已删除)',
    promptPublishDidError: '发布DID出错',
    resolveDidDocumentError: '解析DID文档出错',
    cdeleted: '(评论已删除)',
    sharepost: '分享微说',
    shareChannel: '分享微频',
    error400: '参数错误',
    error500: '服务器错误',
    confirmdeletion: '确认删除这条微说?',
    waitOnChain:
      '温馨提示 - Feeds出版号DID已在ID侧链完成发布，请耐心等待完全确认。',
    takePicture: '拍摄',
    photolibary: '从手机相册选择',
    signedInto: '登录',
    successfully: '成功',
    followed: '订阅',
    unfollowed: '取消订阅',
    filevideodes: '视频播放时长超过15秒限制',
    videoloading: '视频加载中',
    theFeedSource: 'Feeds出版号',
    fulltext: '更多',
    uploading: '发布中',
    downloading: '正在下载',
    mustUpdate: '您的Feeds服务版本需要升级到最新版本，否则您将无法发布内容。',
    downDes: '有文件正在下载，请下载完成后再试。',
    loadingLastestComment: '正在加载最新评论',
    showless: '显示更少',
    showmore: '显示更多',
    unknown: '未知',
    channelAlreadyAdded: '您已经订阅了此频道',
    notFollowYet: '未订阅',
    httperror: '服务异常，稍候再试',
    textcopied: '文本已拷贝',
    connectionError1: '此频道目前离线，稍候再试',
    qRcodePreview: '二维码预览图',
    details: '详情',
    sending: '发送中',
    sendFail: '发送失败',
    retry: '重试',
    sendingTip: '这条微说正在发送，请稍候再试',
    currentPostError: '当前微说出错了',
    sendingErrorTip: '待发送成功后，请重试',
    preferences: '设置',
    posts: '微说',
    confirmDialog: '确认框',
    deletePost: '删除微说',
    deleteComment: '删除微频',
    confirmdeletion1: '确认删除这条微评?',
    bindingCompleted: '绑定成功!',
    error: '错误',
    transMsgTimeout: '服务未响应，请稍候再试',
    advancedSettings: '高级设置',
    normalSettings: '普通设置',
    scannerPrompt: '将二维码放在扫描区域内',
    obtain: '正在获取',
    deleteReply: '删除回复',
    confirmdeletion2: '确认删除这条回复?',
    quantity: '数量',
    connectWallet: '请先连接钱包',
    disconnect: '断开',
    buyNftError: '购买收藏品失败',
    sold: '已出售',
    offTheShelf: '已下架',
    cancelSuccessfully: '取消成功',
    cancellationFailed: '取消失败',
    priceChangeFailed: '更新价格失败',
    publicPasarFailed: '上架到 Pasar 失败',
    nftCreationFailed: 'NFT 创建失败',
    onsale: '出售中',
    state: '状态',
    collectibles: '收藏品（NFT）',
    setPropfileAvatar: '设置您喜欢的头像',
    setAvatar: '设置头像',
    mintSuccess: '创建收藏品成功',
    mintSuccessDesc: '恭喜您，已成功创建收藏品',
    timeout: '超时',
    mintTimeoutDesc: '创建收藏品超时，稍候您可自行确认创建收藏品是否成功',
    uploadingData: '正在上传收藏品数据到 IPFS 网络',
    uploadDataSuccess: '收藏品数据上传到 IPFS 网络完成',
    uploadDataFail: '收藏品数据上传到 IPFS 网络失败',
    mintingData: '正在创建 NFT 收藏品',
    mintDataSuccess: '创建 NFT 收藏品成功',
    mintDataFail: '创建 NFT 收藏品失败',
    settingApproval: '等待授权委托 NFT 收藏品到 Pasar 合约处理',
    agreeApproval: '同意授权委托 NFT 收藏品给 Pasar 合约处理',
    disagreeApproval: '拒绝授权委托 NFT 收藏品给 Pasar 合约，终止出售 NFT 收藏品',
    creatingOrder: '正在创建出售订单',
    createOrderSuccess: '创建出售订单成功',
    createOrderFail: '创建出售订单失败',
    checkingCollectibleResult: '等待创建的 NFT 收藏品在合约中生效',
    checkingOrderResult: '等待创建的 NFT 收藏品订单在合约中生效',
    developLog: '输出日志',
    restartApp: '重启应用',
    restartAppDesc: '切换网络后需要重启应用',
    disconnectWallet: '断开钱包',
    restartAppDescForProvider: '切换提供商后需要重启应用',
    cancelingOrderDesc: '正在取消收藏品订单，请稍候...',
    changingPriceDesc: '正在更新收藏品价格，请稍候...',
    sellingOrderDesc: '正在出售收藏品，请稍候...',
    buyingOrderDesc: '正在购买收藏品，请稍候...',
    buyOrderTimeoutDesc: '购买收藏品超时，稍候您可自行确认购买收藏品是否成功',
    cancelOrderTimeoutDesc: '取消收藏品超时，稍候您可自行确认取消收藏品是否成功',
    changePriceTimeoutDesc: '更新收藏品价格超时，稍候您可自行确认更新收藏品价格是否成功',
    saleOrderTimeoutDesc: '出售收藏品超时，稍候您可自行确认出售收藏品是否成功',
    publishSameDataFailed: '发布到Pasar失败：NFT中已有相同的收藏品',
    savePicture: '保存收藏品',
    savedSuccessfully: '保存成功',
    saveFailed: '保存失败',
    savedDes: '图片保存中，请稍候',
    burnNFTs: '销毁收藏品',
    burningNFTSDesc: '正在销毁收藏品，请稍候',
    burningNFTSTimeoutDesc: '销毁收藏品超时，稍候您可自行确认',
    burnNFTSFailed: '销毁收藏品失败',
    exitApp: '再按一次退出应用！',
    burnNFTSSuccess: '已成功销毁收藏品',
    shareReadMore: '... 点击阅读更多内容',
    sharesharingPost: '这是我从 Feeds 分享的微说',
    shareSharingChannel1: "这是我从 Feeds 分享的微频 '",
    shareSharingChannel2: "' 请订阅后继续阅读",
    shareSharingChannel: '看看来自 Feeds 的这个频道！',
    generateSharingLink: '正在生成分享链接',
    loading: '刷新中...',
    sharePasar: '正在分享收藏品 ',
    parsing: '正在解析...',
    orderCantFind: '在Pasar上找不到该收藏品',
    orderHasBeenSold: '该收藏品已从 Pasar 售出',
    orderHasBeenCancel: '该收藏品已在 Pasar 下架',
    orderInvalid: '该收藏品状态无效',
    internalError: '内部异常',
    transferCollectible: '转让收藏品',
    walletAddress: '钱包地址',
    walletAddressDes: '输入收件人钱包地址',
    tranferNFTSDesc: '正在转让收藏品，请稍候',
    tranferNFTSTimeoutDesc: '转让收藏品超时，稍候您可自行确认',
    tranferNFTSFailed: '转让收藏品失败',
    tranferNFTSSuccess: '转让收藏品成功',
    walletAddressDes1: '收件人钱包地址与收藏品的钱包地址一致',
    walletAddressDes2: '您至少持有 0.01 DIA 才能转让收藏品',
    didUriNull: '服务异常，请稍候再试',
    parseing: '解析中',
    dataStorage: '数据缓存',
    cleanPasarData: '清除 Pasar 缓存数据',
    cleanPasarDataTip: '清除 Pasar 缓存数据后，需要重启应用',
    unpublished: '未公开',
    published: '已公开',
    queryTransactionResult: '已发起交易，正在查询结果',
    sorry: '抱歉',
    auction: '此订单为拍卖订单，暂不支持购买',
    editedContentDes: '确认取消吗？取消将会丢弃您已编辑的内容！',
    editedContentDes1: '确定',
    editedContentDes2: '取消',
    migrateFeedsServiceData: '迁移 Feeds Service 数据到 Hive Vault',
    subscribing: '正在订阅...',
    subscribeSuccess: '订阅成功',
    subscribeFail: '订阅失败（请检查手机网络）',
    didSigninError1: '用户登录时出错',
    didSigninError2: '解析用户数据时出错',
    yes: '是',
    editChannel: '编辑频道',
    likeError: '网络异常，点赞失败',
    likeError1: '点赞失败，请稍候重试',
    errorDes: '抱歉，程序出错了，已向开发团队报告。',
    twitterNotLogin: '您没有登录 Twitter，您要连接您的帐户吗？',
    twitterExpired: 'Twitter 登录已过期，请返回设置页面登录。',
    twitterAuthFailed: 'Twitter 授权失败',
    editPostFail: '更新微说失败',
    createChannelFail: '创建频道失败',
    editChannelFail: '更新频道失败',
    createCommentFail: '创建微评失败',
    editCommentFail: '更新微评失败',
    twitterError: 'Twitter 请求超时',
    twitterError401: 'Twitter 授权失效，请重新授权',
    pinPost: '置顶',
    unpinPost: '取消置顶',
    RedditNotLogin: '您没有登录 Reddit，您要连接您的帐户吗？',
    RedditExpired: 'Reddit 登录已过期，请返回设置页面登录。',
    RedditAuthFailed: 'Reddit 授权失败',
    SubscribeElastosCommunity: '请先在Reddit中订阅Elastos社区',
    redditError: 'Reddit 请求超时',
    twitterError3: '无法解析twitter服务地址，请检查网络后重试',
    unableUnsubscribe: '无法取消订阅自己的频道',
    channelError: '请使用公开此频道时所用的钱包',
    makeTipping: '正在打赏处理中，请稍候',
    tippingFailed: '打赏失败',
    tippingSucess: '打赏成功',
    post: '发布',
    unsubscribeChannel: '取消订阅',
    unsubscribeChannelDes: '您确定要取消订阅频道吗？',
    tipPostTimeoutDesc: '打赏微说超时，稍候您可自行确认'
  },
  app: {
    profileDetails: '档案详情',
    feedSources: '出版号',
    about: '关于 Feeds',
    donation: '捐赠',
    signout: '登出',
    des: '登出成功',
    settings: '设置',
    privacyPolicy: '隐私政策',
    deleteAccount: '删除账号',
    termsOfService: '条款',
    credentials: '凭证'
  },
  SigninPage: {
    signIn: '登录',
    feeds: 'Feeds',
    des: '进行',
    des1: '有意义的对话',
    des2: '您的密钥方能解锁您的数据',
    des3: '没有中心服务器&免审查',
    des4: '您的数据受到高度保护',
    des8: '无需中介',
    des9: '参与资产交易',
    welcome: '#欢迎',
    ownyourdata: '数据自主',
    decentralized: '去中心化',
    secure: '安全',
    freemarket: '#自由市场',
    learnMore: '了解更多',
    des10: 'Web3 社交网络',
    and: '、',
    des11: '登录表明你同意我们的',
    des12: '请选择应用登录方式',
    elastosEssentials: 'Elastos Essentials',
    guest: '访客方式登录'
  },
  FeedsPage: {
    tabTitle1: '主页',
    tabTitle2: '档案',
    tabTitle3: '消息通知',
    tabTitle4: '探索',
    tabTitle5: '时间线',
  },
  HomePage: {
    allempty: '您的时间线是空的',
    emptyDes: '添加新频道或查找要订阅的新频道！',
    exploreFeeds: '探索 Feeds',
    minutesAgo: '分钟前',
    hoursAgo: '小时前',
    daysAgo: '天前',
    onehourAgo: '小时前',
    oneminuteAgo: '分钟前',
    name: '名字',
    creator: '原创者',
    owner: '所有者',
    tokenID: 'Token ID',
    namePlaceholder: '搜索名字',
    creatorPlaceholder: '搜索原创者',
    ownerPlaceholder: '搜索所有者',
    tokenIDPlaceholder: '搜索 Token ID',
    sortBy: '排序',
    recentlyAdded: '最近添加',
    Oldest: '最早的',
    priceLowToHigh: '价格：从低到高',
    priceHightoLow: '价格：从高到低',
    search: '搜索',
    addChannel: '创建频道',
    createPost: '创建微说',
    show: '显示',
    posts: '帖子',
  },
  ProfilePage: {
    myFeeds: '频道',
    following: '订阅',
    myLikes: '赞/收藏',
    moreinfo: '更多信息',
    collectibles: '收藏品',
    createCollectible: '创建收藏品',
    allempty: '没有发现收藏品',
    emptyDes: '创建一个收藏品或从 Pasar 简版市场购买一个！',
    exploreMarketplace: '探索市场',
    connectWallet: '连接钱包',
  },
  MyfeedsComponent: {
    allempty: '没有找到频道',
    emptyDes: '让我们改变现状，从第一个频道开始！',
    createFeed: '添加频道',
    exploreFeeds: '探索 Feeds',
    des: '个频道可以创建',
    des1: '个频道已订阅',
    des2: '个频道已订阅',
    des3: '个频道可以创建'
  },
  FollowingComponent: {
    allempty: '空空如也',
    emptyDes: '去订阅有趣的频道吧',
  },
  LikesComponent: {
    allempty: '还没有收藏',
    emptyLikeDes: '看看时间线上发生了什么！',
    timeline: '时间线',
    exploreFeeds: '探索 Feeds',
    notSupported: '暂时不支持'
  },
  ConfirmdialogComponent: {
    signoutTitle: '登出',
    signoutMessage: '确认要登出吗？',
    deleteAccountTitle: '删除账户',
    deleteAccountMessage: '确认要删除账户并清空全部微说么？',
    des: '请输入 ',
    des1: ' 进行确认'
  },
  SearchPage: {
    title: '频道',
    allempty: '没有找到频道',
    emptyDes: '探索 Feeds 上没有可用的频道。请稍候再来！',
    follow: '订阅',
    following: '已订阅',
    searchFeeds: '搜索',
    confirmTitle: '取消请求',
    des: '无法连接频道'
  },
  ChannelsPage: {
    feeds: '频道',
    tip: '打赏',
    allempty: '空空如也',
    emptyDes: '该频道尚未发布任何微说',
    noSubscription: '没有订阅',
    subscribeChannel: '订阅此频道可查看帖子',
    viewall: '查看全部',
    allempty1: '频道未订阅',
    emptyDes1: '订阅以查看此频道的内容',
    pinnedPost: '置顶微说',
    PinPostError: '置顶微说失败',
    UnPinPostError: '取消置顶微说失败'
  },
  NotificationPage: {
    allempty: '这里什么都找不到',
    emptyDes: '别担心，您所有的通知都会显示在这里！',
    followedFeed: '订阅了您的频道',
    likedPost: '赞了微说',
    commentPost: '在微说留了言',
    likedComment: '赞了微评',
    deleteNotification: '删除通知',
  },
  CreatenewfeedPage: {
    createNewFeed: '新建频道',
    profileImage: '频道头像',
    des: '为您的频道选择头像',
    channelSource: '出版号DID',
    name: '频道名称',
    displayName: '频道显示名称',
    description: '频道描述',
    createFeed: '创建',
    tipMsg: '请选择一个头像',
    tipMsg1: '请输入频道名称',
    tipMsg2: '请输入频道描述信息',
    tipMsg3: '无效的参数',
    tipMsgLength: '描述长度最大为128个字符',
    inputName: '输入频道名称',
    inputFeedDesc: '输入频道描述',
    createfeedsuccess: '创建频道成功!',
    tipMsgLength1: '名称长度最大为32个字符',
    alreadyExist: '频道已存在，请更改名称后重试',
    feedMaxNumber: '最多可创建5个频道',
    des1: '在探索 Feeds 中显示频道，以供用户发现和订阅。',
    tippingAddress: '打赏地址',
    tippingAddressDes: '输入打赏地址',
    inputDisplayName: '请输入频道显示名称',
    channelnamedes: '创建后不允许修改',
    displaynamedes: '创建后允许修改',
    tipMsgLength2: '显示名称长度最大为32个字符'
  },
  ProfileimagePage: {
    title: '选择头像',
    confirm: '确认',
    avatarEorr: '不支持GIF文件类型，请重新选择'
  },
  ProfiledetailPage: {
    profileDetails: '档案详情',
    name: '名称',
    did: 'DID身份码',
    description: '个人描述',
    telephone: '号码',
    email: '邮箱',
    location: '国家/地区',
    didinfo: 'DID信息',
    publisheraccount: '出版号',
    collections: '收藏品',
    basicAccount: '基本帐户',
    features: '特征',
    features1: '● 买卖收藏品',
    features2: '● 阅读订阅频道的帖子',
    features3: '● 评论帖子',
    features4: '● 创建最多5个频道',
    features5: '● 发布无限量的帖子',
    features6: '● 向订阅者发布NFT销售',
    upgradeToPublisherAccount: '升级到发布者帐户'
  },
  DevelopPage: {
    develop: '开发',
    clearAllData: '清除所有数据',
  },
  AboutPage: {
    about: '关于 Feeds',
    version: '版本',
    website: '团队网址',
    disclaimer: '免责声明(Feeds)',
    disclaimerNft: '免责声明(Pasar 简版)',
    help: 'Feeds使用手册',
    email: '邮箱',
    telegram: '电报'
  },
  PostdetailPage: {
    postview: '微说详情',
    replyingto: '回复给',
  },
  AddServerPage: {
    tipMsg: 'URL地址可能错误',
    feedsSource: 'Feeds出版号: ',
    serverWentOffline: ' 已变成离线',
    serverAlreadyAdded: '该Feeds出版号已经在您的订阅列表中了',
    scanimg: '导入二维码图片',
    tipMsg1: '未发现二维码',
  },
  CreatenewpostPage: {
    post: '发布',
    des: '在此输入...',
    addingPost: '新建微说',
    tipMsg: '请输入内容',
    tipMsg1: '您的微说已经发布成功',
    tipMsg2: '视频处理中',
    twitterDes: '发布到:',
    twitterDes1: '目前，只支持文本格式。任何图像或视频都不会发布到Twitter、Reddit。',
  },
  DIDdata: {
    Notprovided: '',
    NotprovidedfromDIDDocument: '信息获取中',
    NoDescription: '暂无个人简介和兴趣爱好信息',
  },
  CommentPage: {
    publishComment: '发布微评',
    inputComment: '输入评论',
    tipMsg1: '您的评论已经发布成功',
  },
  DisclaimerPage: {
    title: '免责声明',
    declaration: '点击“同意”，即表示您接受并同意我们以下条款：',
    copyright:
      'Feeds 由 Trinity Tech Co.，Ltd. 及其相关团队（“Trinity” 或 “我们”）拥有并运营，并且您同意遵守以下 Feeds 应用程序的特定条款（统称为“条款”）。',
    bodyText:
      '通过使用我们的 Feeds 应用程序、其内容和服务（以下简称“应用程序”），您同意对您使用该应用程序以及您发布和提供的任何内容承担全部责任，包括遵守法律、法规和规定。 您应该只发布和提供可以与他人共享的内容。 您必须年满13周岁才能使用该应用程序，或者在您所在的辖区（即市、省或国家）不再被视为未成年人。 我们不认可、支持、说明或保证应用程序上任何内容的完整性、真实性、准确性或可靠性。 您在此确认：在使用该应用程序时，您可能会接触到有害的、不准确的或不适当的内容。',
    acceptText: '同意',
    denyText: '不同意',
  },
  DonationPage: {
    donation: '捐赠',
    donate: '捐赠',
    description:
      'Feeds是一款非盈利社交应用项目，致力于持续改进用户体验和集成更多价值特性。您的捐献是对我们持续发布新版本的一种鼓励，同时也是一份责任提醒。',
    receiver: '接收者',
    amount: '数量',
    enterAmount: '输入ELA数量',
    memo: '备注',
    defaultMemo: '捐赠Feeds项目',
    supportFeeds: '支持Feeds',
    elaAddress: 'ELA地址',
    channelName: '频道名称',
  },
  GalleriahivePage: {
    title: '登录 Hive 中',
    description:
      '正在等待 Essentials 授权登录 Hive， 请稍候....',
    buttonTitle: '开始',
    synchronizingData: '正在同步数据',

    preparingData: '正在准备数据',
    creatingScripting: '正在创建脚本',
    synchronizingChannelData: '正在同步频道数据',
    synchronizingPostData: '正在同步微说数据',
    synchronizingCommentData: '正在同步微评数据',
    synchronizingOtherData: '正在同步其他数据',

    synchronizingComplete: '同步完成，开始使用',
    buttonTitle1: '请重试',
    welcomeHive: '开始使用',
    titleFail: '登陆 Hive 失败',
    failDes0: 'Essentials 授权登录 Hive 失败',
    failDes1: '请检查手机网络',
    failVaultNotFound: '请启用Hive存储',
    authorization: '授权',
    description1: '正在连接 Hive 节点...',
    authorized: '已授权',
    description2: '成功登录到 Hive 节点!'
  },
  ErrorInfo: {
    alreadyExists: '内容已经存在',
    notExists: '内容不存在',
    notAuthorized: '操作异常',
    wrongState: '状态异常',
    tokenExpired: '令牌过期',
    internalError: '服务器内部异常',
    invalidParam: '参数无效',
    invalidChallengeResponse: '登录鉴权失败',
    invalidVerifiableCredential:
      '服务器无法验证凭证信息，请检查您的DID是否已发布到侧链',
    unsupportedRequests: '不支持的请求',
    needUpdateServerVersion: '需要升级',
    UnknownError: '未知错误',
    UnimplementedError: '未实现',
    NotFoundError: '未找到',
    InvalidArgument: '无效参数',
    PointerReleasedError: '指针释放出错',
    DevUUIDError: '设备的UUID错误',
    FileNotExistsError: '文件不存在',
    CreateDirectoryError: '创建目录出错',
    SizeOverflowError: '大小溢出错误',
    StdSystemError: 'StdSystem出错',
    OutOfMemoryError: '服务端内存溢出',
    DidNotReady: 'DID未准备好',
    InvalidAccessToken: '无效的访问Token',
    NotAuthorizedError: '未授权',
    CarrierSessionInitFailed: 'Carrier Session: 初始化失败',
    CarrierSessionConnectFailed: 'Carrier Session: 链接失败',
    CarrierSessionCreateFailed: 'Carrier Session: 创建失败',
    CarrierSessionAddStreamFailed: 'Carrier Session: 添加Stream失败',
    CarrierSessionTimeoutError: 'Carrier Session: 超时',
    CarrierSessionReplyFailed: 'Carrier Session: 回复失败',
    CarrierSessionStartFailed: 'Carrier Session: 启动失败',
    CarrierSessionBadStatus: 'Carrier Session: 糟糕的Status',
    CarrierSessionDataNotEnough: 'Carrier Session: 数据不够',
    CarrierSessionUnsuppertedVersion: 'Carrier Session: 版本不支持',
    CarrierSessionReleasedError: 'Carrier Session: 发布出错',
    CarrierSessionSendFailed: 'Carrier Session: 发送失败',
    CarrierSessionErrorExists: 'Carrier Session: 已存在',
    MassDataUnknownReqFailed: 'MassData: 未知请求失败',
    MassDataUnmarshalReqFailed: 'MassData: Unmarshal请求失败',
    MassDataMarshalRespFailed: 'MassData: Marshal响应失败',
    MassDataUnsupportedVersion: 'MassData: 版本不支持',
    MassDataUnsupportedAlgo: 'MassData: Algo不支持',
    STREAM_STATE_DEACTIVATED: 'Stream State: 已停用',
    STREAM_STATE_CLOSED: 'Stream State: 已关闭',
    STREAM_STATE_ERROR: 'Stream State: 出错',
    WRITE_DATA_ERROR: '写数据出错',
    SESSION_CREATE_TIMEOUT: 'Session: 创建超时',
    SESSION_NEW_SESSION_ERROR: 'Session: 创建Session出错',
    SESSION_ADD_STREAM_ERROR: 'Session: 添加Stream出错',
    SESSION_REQUEST_ERROR: 'Session: 请求出错',
    SESSION_START_ERROR: 'Session: 启动出错',
    HIVE_ERROR_507: '抱歉，可用的Hive存储空间不足',
    HIVE_ERROR_undefined: '网络错误，请重试',
    HIVE_ERROR_URL: '获取DID绑定的Hive url失败'
  },
  EidtchannelPage: {
    title: '编辑频道信息',
    des: '提示框',
    des1: '此频道已经在探索Feeds公开，需要先从探索Feeds下架，才可以修改。',
    des2: '此频道已经是收藏品且在探索Feeds公开，需要从探索Feeds下架，销毁收藏品才可以修改。',
    des3: '此频道已经是收藏品，需要销毁收藏品后才可以修改。',
    updateChannelDesc: '正在更新频道信息...，请稍候..',
    updateChannelTimeoutDesc: '更新频道信息超时，稍候您可自行确认',
    updateChannelSuccess: '已成功更新频道信息',
    updateChannelFailed: '更新频道信息失败',
    updateChannelDesc1: '更新频道信息...，请稍候..',
    updateChannelDesc2: '当前频道信息与公开的频道信息不一致，请及时同步数据。',
    updateChannelDesc3: '同步数据'
  },
  EditPostPage: {
    title: '编辑微说',
  },
  EditCommentPage: {
    title: '编辑微评',
    saveChanges: '保存修改',
    notModifiedYet: '您还没有修改评论内容',
  },
  SettingsPage: {
    devmode: '开发者模式',
    cleandata: '清空数据',
    des: '确认要清空数据吗？',
    des1: '清空数据成功',
    hidedeletepost: '显示已删除的微说',
    hidedeletecomment: '显示已删除的评论',
    on: '开',
    off: '关',
    'language-setting': '语言',
    darkMode: '主题',
    drak: '深色',
    light: '浅色',
    'elastos-api-provider': 'API 提供商',
    'elastos-api-provider-des': 'Elastos & IPFS API 提供商',
    'elastos-io-des': 'Gelaxy',
    'trinity-tech-cn-des': 'Trinity Tech',
    'ipfs-provider': 'IPFS API 提供商',
    'ipfs0-provider-des': '主网关',
    'ipfs1-provider-des': '辅助网关',
    'ipfs2-provider-des': '第三级网关',
    'assistPasar-provider': 'Assist API 提供商',
    /*********** Elastos API provider page ***********/
    elastosapiprovider: 'API 提供商',
    'developer-setting': '开发者选项',
    'advanced-setting': '高级设置',
    'configure-network': '启用中网络',
    'choose-network': '选择网络',
    'setPasarListGrid': '列表视图-Pasar',
    hideAdult: '安全模式',
    appPreferences: 'App首选项',
    appPreferencesDes: '管理应用首选项',
    developer: '开发者模式',
    developerDes: '环境设置',
    languageDes: '全局语言设置',
    apiProviderDes: '配置API提供商',
    dataMigration: '数据迁移',
    dataMigrationDes: 'Feeds Service to Hive vault',
    networkDes: '网络配置',
    interfaceDes: '接口测试',
    interface: '接口',
    databaseCollections: '数据库集合',
    databaseCollectionsDes: '删除所有 collections',
    connections: '连接',
    connectionsDes: '管理帐户连接'
  },
  FeedinfoPage: {
    title: '频道详情',
    feeds: '频道',
    updatetime: '更新时间',
    feedsQrcode: '频道二维码'
  },
  CommentlistPage: {
    title: '评论详情',
    replydeleted: '(回复已删除)',
    editreply: '编辑回复',
    deletereply: '删除回复',
  },
  FeedspreferencesPage: {
    title: '频道设置',
    des: '在探索Feeds中显示',
    des1: '发布收藏品到频道',
    des2: '创建频道',
    des3: '取消公开频道',
    des4: '是否把频道从探索Feeds广场下架',
    des5: '是',
    des6: '否',
    burnNFTs: '取消公开频道 ',
    burningNFTSDesc: '取消公开频道, 请稍候',
    burningNFTSTimeoutDesc: '取消公开频道超时, 稍候您可自行确认',
    burnNFTSFailed: '取消公开频道失败',
    burnNFTSSuccess: '取消公开频道成功',
  }, /***NFT*/
  MintnftPage: {
    title: '创建收藏品',
    nftAsset: '资产',
    nftAssetPlaceholder: '添加资产',
    nftName: '名称',
    nftNamePlaceholder: '添加名称',
    nftDescription: '描述',
    nftDescriptionPlaceholder: '添加描述',
    putOnSale: '出售',
    nftType: '类型',
    single: '唯一',
    multiple: '多个',
    nftRoyalties: '版税',
    nftRoyaltiesPlaceholder: '版税（建议：10％）',
    nftQuantity: '数量',
    nftQuantityPlaceholder: '输入数量',
    create: '创建',
    nftAmount: '金额',
    nftFixedAmount: '输入固定金额',
    nftMinimumAmount: '输入最低金额',
    nftSellMethod: '出售方式',
    nftfixedPrice: '固定价格',
    nfthighestBid: '最高出价',
    nftExpirationDate: '截止日期',
    nftExpirationDateDes: '选择截止日期',
    priceErrorMsg: '出售价格应大于0',
    priceErrorMsg1: '新出售价格应与现有出售价格不同',
    royaltiesErrorMsg: '版税应为介于0和15%之间的整数',
    quantityErrorMsg: '数量必须是大于0的整数',
    quantityErrorMsg1: '数量(整数)不能超过最大值',
    fileTypeDes: '支持图片格式 -  ',
    avatar: '头像',
    fileTypeDes1: '600 * 600 唯一像素标准',
    fileTypeDes2: '内容尺寸小于等于5M',
    fileTypeDes3: '建议的GIF动画长度：小于10秒',
    general: '通用',
    assetType: '资产类型',
    adult: '明确和敏感的内容',
    video: '视频',
    fileTypeDes4: '支持视频格式 -',
    fileTypeDes5: '视频播放时长超过60分钟限制',
    audio: '音频',
    fileTypeDes6: '支持音频格式 -',
  },
  AssetdetailsPage: {
    title: '收藏品明细',
    asset: '资产',
    contractDetails: '合约明细',
    owner: '所有者',
    name: '名称',
    description: '描述',
    quantity: '数量',
    dateCreated: '创建日期',
    stickerContractAddress: '铸币合约地址',
    pasarContractAddress: '交易商场合约地址',
    tokenID: 'Token ID',
    purchaseInfo: '购买信息',
    creator: '原创者',
    datePurchased: '购买日期',
    price: '价格',
    currency: '货币',
    type: '类型',
    burn: '销毁',
    history: '历史',
    contract: '合约',
    state: '状态',
    royalties: '版税',
    priceErrorMsg: '出售价格不能为负值',
    saleType: '出售类型',
    firstSale: '首次销售',
    secondarySale: '二次转售',
    dateoNMarket: '上架日期',
    pasarExplorer: '查看更多在 Pasar Explorer'
  },
  BidPage: {
    title: '收藏品明细',
    blockchain: '区块链',
    buy: '购买',
    myBid: '我的出价',
    bid: '出价',
    bidPlaceholder: '输入出价金额',
    minimumBid: '最小出价',
    currentBid: '当前出价',
    changePrice: '更新价格',
    cancelOrder: '取消出售',
    des: '购买前请确认创建者地址',
    verifiedCreator: '原创者',
    verified: '(已验证)'
  },
  CollectionsPage: {
    title: '收藏品',
    created: '已创建',
    purchased: '已购买',
    likes: '喜欢',
    emptylikes: '给喜欢的收藏品点个赞',
    emptypurchased: '去购买收藏品吧',
    onSale: '出售收藏品',
    putOnSale: '出售',
    price: '价格',
    details: '详情',
    sale: '出售'
  },
  ExplorefeedsPage: {
    pasar: 'Pasar 简版',
  },
  LearnmorePage: {
    title: '了解更多',
    next: '下一步',
    skip: '跳过',
  },
  NftdisclaimerComponent: {
    title: 'Feeds 收藏品 免责声明',
    des: '对于因使用 Feeds Pasar（及其供应商）提供的服务而导致的任何损失，Feeds 团队概不负责，包括但不限于直接、间接、附带、惩罚性和已发生的损失：',
    des1: '1. 用户错误，如密码丢失、非法交易、个人信息泄露、地址输入错误等；',
    des2: '2. 数据丢失、损坏或服务器故障；',
    des3: '3. 因业务中断造成的损失；',
    des4: '4. 漏洞或任何类型的程序问题，如钱包和智能合约，以及任何其他与区块链或加密资产相关的功能问题，如分叉和技术节点问题；',
    des5: '5. 任何未经授权的第三方活动，包括但不限于对服务或加密资产使用病毒、钓鱼、暴力破解或其他攻击方式；',
    des6: '6. 因使用本平台服务造成的损失包括但不限于法律问题、合同责任问题、经济问题等；',
    des7: '7. 因侵权（包括疏忽）、违约或任何其他原因造成的任何间接、特殊或附带的损失或损害，无论该等损失或损害是否可由 Feeds 团队合理预见，也无论 Feeds 团队已被提前通知此类损失或损害的可能性；',
    des8: '8. 任何非由 Feeds 团队造成的与 Feeds Parser 平台提供的服务相关的任何其他损失；',
    des9: '9. Feeds 团队对 Feeds Parser 平台服务的使用不作任何明示或暗示的保证，包括但不限于：本平台所提供服务的适用性、错误或遗漏、连续性、准确性、可靠性，以及对特定用途的适用性。此外，Feeds 团队对 Feeds Parser 平台提供的服务相关的技术和信息的有效性、准确性、正确性、可靠性、质量、稳定性、完整性或及时性不作任何承诺或保证。使用本平台提供的服务是用户个人选择，风险自负。Feeds 团队不对数字资产的市场、价值或价格做出任何明确或暗示的保证。请注意，数字资产市场不稳定，市场随时可能出现波动或崩盘。交易数字资产是用户的个人选择，风险自负，用户应能够独立承担任何潜在的损失。',
    des10: 'Feeds 团队不会就 Feeds Pasar（及其供应商）提供的服务是否符合任何特定司法管辖区的法律法规或相关政策要求提供任何评估或评论。强烈建议用户严格遵守各自司法管辖区的法律。'
  },
  NftavatarlistPage: {
    title: '收藏品头像'
  },
  AssistpasarPage: {
    title: 'Assist Pasar API',
    assistProviderDes: '用于读取数据的辅助 Assist Pasar 服务',
  },
  ProfilenftimagePage: {
    title: '选择收藏品'
  },
  ChannelcollectionsPage: {
    title: '频道收藏品',
    openCollections: '公开收藏品',
    cancelPublicCollections: '取消公开收藏品'
  },
  GalleriachannelPage: {
    title: '频道收藏品',
    publicGallericaFailed: '公开到探索频道失败',
    mintingData: '正在创建频道收藏品',
    settingApproval: '等待授权委托收藏品到 Galleria 合约处理',
    creatingOrder: '正在创建出售订单',
    createOrderSuccess: '创建出售订单成功',
    createOrderFail: '创建出售订单失败',
    checkingCollectibleResult: '等待创建的频道收藏品在合约中生效',
    mintSuccess: '创建频道收藏品成功',
    mintSuccessDesc: '恭喜您，创建频道收藏品成功',
    // publishSameDataFailed: '公开频道失败：探索频道中已有相同的频道',
    publishSameDataFailed: '公开频道失败：请重新连接钱包后再操作',
    signatureError: '签名数据失败，请重试',
    signatureError1: '请确保Essentials登录的DID与Feeds登录的DID一致。'
  },
  ScanPage: {
    title: '扫描器',
    noQrErr: '在所选图片中未检测到二维码。 请再试一次。',
    scanErr: '扫描您选择的照片时出了点问题。 请再试一次。',
    galleryErr: '访问图库时出了点问题。 请再试一次。',
  },
  ApppreferencesPage: {
    title: 'App首选项',
  },
  buyStorageSpaceDialog: {
    title: '存储不足',
    noBuy: '不买',
    buy: '买'
  },
  toast: {
    linkageInvalidParams: '分享链接无效，无法打开链接'
  },
  ConnectionsPage: {
    title: '连接',
    addConnection: '添加连接',
    addConnectionDes: '整合新账户',
    twitter: '推特',
    connected: '已连接',
    disconnect: '断开',
    reddit: 'Reddit'
  },
  UserlistPage: {
    title: '订阅者列表',
    emptyDes: "还没有人订阅此频道"
  },
  DeveloperPage: {
    confirmTitle: "删除数据",
    des: "确认删除所有数据吗？"
  },
  EditprofileinfoPage: {
    title: '编辑档案详情',
    des: '描述',
    inputName: '添加名字',
    inputDes: '添加描述',
    inputTelephone: '添加手机号',
    inputEmail: '添加邮箱',
    notChange: '资料未发生变化'
  },
  UserprofilePage: {
    title: '用户档案',
    allempty: '没有找到频道',
    emptyDes: '让我们改变现状，从第一个频道开始！'
  },
  UserprofileinfoPage: {
    title: '用户档案详情'
  },
  PosttiplistPage: {
    title: '微说打赏列表',
    tip: '打赏',
    allempty: '还没有人打赏'
  }
};
